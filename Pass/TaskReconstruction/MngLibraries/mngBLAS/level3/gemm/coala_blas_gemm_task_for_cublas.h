#ifndef _COALA_BLAS_GEMM_TASK_FOR_CUBLAS_H
#define _COALA_BLAS_GEMM_TASK_FOR_CUBLAS_H
...
*************************************************/
class CoalaBlasGemmTask4Cublas: public  CoalaBlasGemmTask
{
    private:
	//构造函数(不可公开访问)
	CoalaBlasGemmTask4Cublas(){}
   ...
   //参数set方法(私有)
    ...
    public:
	//构造函数（重载）
   	CoalaBlasGemmTask4Cublas(CallInst * _callee, COALA_BLAS_ROUTINES_CODE const _code, size_t const _taskid)
	{	
		routine_callee 	= _callee;
		routine_code 	= _code;
		taskid 			= _taskid;	
		routine_callee_params.emplace("Layout",setLayout());
		routine_callee_params.emplace("Handle",setHandle());
		...
		_setDevMalcCalleeInfo(getRoutineParam("MatA"));
		_setDevMalcCalleeInfo(getRoutineParam("MatB"));
		_setDevMalcCalleeInfo(getRoutineParam("MatC"));
		_setHost2DevCalleeInfo(getRoutineParam("MatA"));
		_setHost2DevCalleeInfo(getRoutineParam("MatB"));
		_setHost2DevCalleeInfo(getRoutineParam("MatC"));
		_setDev2HostCalleeInfo(getRoutineParam("MatA"));
		_setDev2HostCalleeInfo(getRoutineParam("MatB"));
		_setDev2HostCalleeInfo(getRoutineParam("MatC"));
		_setDevFreeCalleeInfo(getRoutineParam("MatA"));
		_setDevFreeCalleeInfo(getRoutineParam("MatB"));
		_setDevFreeCalleeInfo(getRoutineParam("MatC"));
	}
